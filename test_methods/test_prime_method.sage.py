# This file was *autogenerated* from the file test_prime_method.sage
...
def main():
	...
	for test_num in xrange(Integer(4),Integer(5)):
		choose_test(test_num)
		print 'Using test \# '+str(test_num)+' .........'
		for threshold in xrange(Integer(3),Integer(11)):
			...
			try:
				t0 = time.time() #initial time
				omegap = \
				method6(zip(range(Integer(1),threshold+Integer(1)),[len(q)]*len(range(Integer(1),threshold+Integer(1)))))
				omegap = map(itemgetter(-Integer(1)), omegap)
				omega6 = (z-gamma)*reduce(operator.mul, omegap, Integer(1))
				t1 = time.time()
...
def choose_test(test_number):
	...
	if test_number==Integer(0):
		delta = [-Integer(1)/Integer(2), Integer(1)/Integer(2)]
		q = [Integer(1)/Integer(4), Integer(1)/Integer(4)]
	
	if test_number==Integer(1):
		delta = [-Integer(3)/Integer(4), -Integer(1)/Integer(4), Integer(1)/Integer(2)]
		q = [Integer(1)/Integer(18), Integer(1)/Integer(18), Integer(1)/Integer(4)]
	
	if test_number==Integer(2):
		delta = [-Integer(3)/Integer(4), Integer(0)]
		q = [Integer(1)/Integer(18),Integer(1)/Integer(9)]
	
	if test_number==Integer(3):
		delta = [-Integer(3)/Integer(4),-Integer(1)/Integer(4),Integer(1)/Integer(6),Integer(3)/Integer(4)]
		q = [Integer(1)/Integer(18),Integer(1)/Integer(19),Integer(1)/Integer(20),Integer(1)/Integer(30)]
	
	if test_number==Integer(4):
		delta = [-n/Integer(8) for n in xrange(Integer(1),Integer(8))]
		delta += [n/Integer(8) for n in xrange(Integer(1),Integer(8))]
		q = [Integer(1)/Integer(100)] * len(delta)

	return None

phi_j = lambda j: delta[j] + q[j]**Integer(2)*z/(Integer(1)-delta[j].conjugate()*z) #phi_j(z)
...
def method1(genus, product_threshold): 
	G = FreeGroup(genus)
	omegap = Integer(1) #Initial choice for omegap, multiply by stuff laters
	for k in range(Integer(1),product_threshold+Integer(1)):
...
def method2(genus, word_length):
	...
	return reduce(operator.mul, output,Integer(1))
...
def method3(genus, word_length):
	...
	output = map(itemgetter(-Integer(1)),output)
	return reduce(operator.mul, output,Integer(1))
...
def method4(genus, threshold):
	G = FreeGroup(genus)
	pool = []
	for k in xrange(Integer(1),threshold+Integer(1)): pool += Words(genus,k)
